...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record screen-only Tekken 8 clips and temporal pseudo-labels.")
    parser.add_argument("--screen-config", type=Path, default=Path("config/live_screen.yaml"))
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/live_capture"))
    ...
    if args.seconds <= 0 or args.record_fps <= 0:
        parser.error("--seconds and --record-fps must be positive")

    session = args.output_dir / time.strftime("%Y%m%d_%H%M%S")
    frames_dir = session / "frames"
    frames_dir.mkdir(parents=True, exist_ok=False)
    backend = DxcamScreenStateBackend(
        config_path=args.screen_config,
        p1_on_left=args.facing == 1,
    )
    ...
    try:
        with metadata_path.open("w", encoding="utf-8") as metadata:
            while time.perf_counter() < deadline:
                ...
                encoded = cv2.cvtColor(resized, cv2.COLOR_RGB2BGR)
                if not cv2.imwrite(str(frames_dir / frame_name), encoded, [cv2.IMWRITE_JPEG_QUALITY, 88]):
                    raise RuntimeError(f"failed to write {frame_name}")
        ...
        json.dumps(
            {
                ...
                "width": args.width,
                "height": args.height,
                "calibration_source": backend.config.get("calibration_source", "unspecified"),
                "p1_on_left": args.facing == 1,
            },
            indent=2,
        ),
